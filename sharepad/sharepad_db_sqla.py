...
def init_ingredienttypes_table(s):
    for i in ingr_types:
        t = IngredientType(Name=i[0], DisplayName=i[1])
        s.add(t)

def get_ingredienttype_id(s, ingredienttype):
    r = s.query(IngredientType).filter(IngredientType.Name == ingredienttype).one()
    try:
        return r.Id
    except TypeError:
        return None

def init_ingredients_table(s):
    for i in ingredients:
        t = Ingredient(Name=i[0], DisplayName=i[1], Type=get_ingredienttype_id(s, i[2]))
        s.add(t)

def get_ingredient_id(s, ingredient):
    id = s.query(Ingredient).filter(Ingredient.Name == ingredient).one().Id
    if id is None:
        print "Ingredient %s not found in db" % (ingredient,)
    return id

def init_styles_table(s):
    for i in styles:
        t = Style(Name=i[0], DisplayName=i[1])
        s.add(t)
        
def init_styles_ingredients_table(s):
    for i in styles_ingredients:
        for j in styles_ingredients[i]:
            t = StyleIngredient(Style=get_style_id(s,i), Ingredient=get_ingredient_id(s,j))
            s.add(t)

def add_pizza(s, pizza):
    pizza_id = None
    now = datetime.datetime.now()
    t = Pizza(CreatedOn=now, CreatedBy="TESTUSER", Style=get_style_id(pizza['style']))
    pizza_id = s.add(t)
    print  "added ", pizza_id
    ingredients = list(itertools.chain.from_iterable(pizza['ingredients'].values()))
    for i in ingredients:
        ingr_id = get_ingredient_id(i)
        t = PizzaIngredient(pizza_id, ingr_id)
        s.add(t)
        print pizza
...
def get_style_id(s, style):
    r = s.query(Style).filter(Style.Name == style).one()
    return r.Id
...
def create_tables():
    ...
    pass # no creation needed for sqlalchemy

def init_database():
    """Initialise the database"""
    s=session()
    init_ingredienttypes_table(s)
    init_ingredients_table(s)
    init_styles_table(s)
    init_styles_ingredients_table(s)
    s.commit()
